...
class Company(Base):
    ...
    def give_freebie(self, dev, item_name, value):
        ...
        print(
            f"{new_freebie.dev}, {new_freebie.item_name}, {new_freebie.value}, {new_freebie.company}"
        )

    @classmethod
    def oldest_company(cls):
        oldest_company = (
            session.query(Company).order_by(desc(Company.founding_year)).first()
        )
...
class Dev(Base):
    ...
    def received_one(self, item_name):
        print(f"Checking for {item_name} in freebies:")
...
class Freebie(Base):
    ...
    def print_details(cls):
        ...
        for freebie in freebies:
            detail = f"{freebie.dev.name} owns a {freebie.item_name} from {freebie.company.name}"
